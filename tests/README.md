...
|---|---|---|
| `test_datapack.py` | Проверки из `tools/interlude/checks.py`: битые ссылки на предметы, NPC, скиллы, магазины, мультиселлы, телепорты, зоны спавна, диалоги квестов, кнопки выключенных скриптов и меню GM без выключенных систем; монстры квестов расставлены в мире, у лидеров есть миньоны Interlude; XML по XSD; контент поздних хроник в магазинах, дропе, рецептах, спавнах, деревьях скиллов, загрузчиках скриптов; конфиги Interlude; данные ботов (снаряжение, реплики, города). Плюс тесты на сами проверки: каждой подсовывают сломанный датапак и ждут, что она это заметит | нет aCis в `../acis_public-master` |
