...
|---|---|---|
| `test_datapack.py` | Проверки из `tools/interlude/checks.py`: битые ссылки на предметы, NPC, скиллы, магазины, мультиселлы, телепорты, зоны спавна и диалоги квестов; монстры квестов расставлены в мире; XML по XSD; контент поздних хроник в магазинах, дропе, рецептах, спавнах, деревьях скиллов, загрузчиках скриптов; конфиги Interlude; данные ботов (снаряжение, реплики, города). Плюс тесты на сами проверки: каждой подсовывают сломанный датапак и ждут, что она это заметит | нет aCis в `../acis_public-master` |
...
| `test_tools.py` | Функции скриптов `tools/interlude`, которые переписывают файлы датапака (`build_html_links`, `build_spawns`, `fix_floating_spawns`, `port_quest_html`, `kamael`), и то, что сгенерированные спавны содержат нужных NPC | никогда |
