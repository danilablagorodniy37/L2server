...
public final class AltarsOfSacrifice extends AbstractNpcAI {
	...
	private static final Logger LOG = LoggerFactory.getLogger(AltarsOfSacrifice.class);
	...
	private static final class Altar {
		...
		private Altar(final ILocational middlePoint, final int... bossNpcIds) {
			_middlePoint = middlePoint;
			_bossNpcIds = bossNpcIds;
			_spawnedBoss = null;
		}
		
		private void spawnBoss() throws Exception {
		...
		private boolean hasBosses() {
			return _bossNpcIds.length > 0;
		}
		
		private boolean isBossFighting() {
	...
	public boolean unload(boolean removeFromList) {
		LOG.info("Unloading altars due to script unloading.");
	...
	public String onEvent(String event, L2Npc npc, L2PcInstance player) {
		if (isSpawnBossEvt(event)) {
			...
			try {
				...
				LOG.warn("Failed to spawn altar boss.", ex);
