...
public final class TowerOfNaia extends AbstractNpcAI {
	...
	private static final Map<Integer, int[]> DOORS = new HashMap<>();
	private static final Map<Integer, Integer> ZONES = new HashMap<>();
	private static final Map<Integer, int[][]> SPAWNS = new HashMap<>();
	...
	private int getSporeGroup(int sporeId) {
		int ret = switch (sporeId) {
			case SPORE_FIRE, SPORE_WATER -> 0;
			case SPORE_WIND, SPORE_EARTH -> 1;
			default -> -1;
		};
		return ret;
	}
	...
	private void markElpyRespawn() {
		final long respawnTime = (getRandom(43200, 216000) * 1000L) + System.currentTimeMillis();
