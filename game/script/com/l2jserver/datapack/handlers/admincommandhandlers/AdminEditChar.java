...
public class AdminEditChar implements IAdminCommandHandler {
	...
	private void listCharacters(L2PcInstance activeChar, int page) {
		...
		final PageResult result = HtmlUtil.createPage(players, page, 20, i -> ("<td align=center><a action=\"bypass -h admin_show_characters " + i + "\">Page " + (i + 1) + "</a></td>"), player -> {
            String sb = "<tr>" +
                    "<td width=80><a action=\"bypass -h admin_character_info " + player.getName() + "\">" + player.getName() + "</a></td>" +
                    "<td width=110>" + ClassListData.getInstance().getClass(player.getClassId()).getClientCode() + "</td><td width=40>" + player.getLevel() + "</td>" +
                    "</tr>";
			return sb;
		});
