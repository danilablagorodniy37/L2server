...
## Боты в городах
Персонажи без клиента гуляют по крупным городам и переписываются в чате о сервере
(локации под уровень, живые рейд-боссы, владельцы замков, цены). Они помнят события:
убитых рейд-боссов, смерти и уровни рядом, осады, олимпиаду, новые кланы — и
обсуждают их, в том числе диалогами в несколько реплик. У каждого своя манера письма. Включается в
`game/config/phantoms.properties`, скрипт `game/script/com/l2jserver/datapack/custom/phantoms`,
данные в `game/data/phantoms`: реплики `phrases.txt`, локации `zones.txt`,
снаряжение `gear.txt` (генерирует `tools/interlude/build_phantom_gear.py`).
Боты — обычные персонажи служебного аккаунта, вход в него закрыт. Их реплики
пишутся в `game/logs/chat`.
